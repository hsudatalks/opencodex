...
export const ru: Record<TKey, string> = {
  ...
  "auth.adminTokenTitle": "Токен администратора Univers Gateway (OPENCODEX_ADMIN_AUTH_TOKEN)",
  ...
  "app.logoAria": "Логотип Univers Gateway",
  ...
  "startup.subtitle": "Проверьте, сможет ли Codex подключиться к Univers Gateway после перезагрузки, прежде чем локальный прокси вызовет бесконечное переподключение.",
  ...
  "startup.summary.protected": "Univers Gateway будет доступен после перезагрузки",
  ...
  "startup.riskDetailCustomLocal": "Codex направлен на пользовательский локальный шлюз. Univers Gateway не может управлять или проверять его перезапуск.",
  ...
  "startup.codexRuntime.clampHidden": "Некоторые уровни рассуждений скрыты, потому что Univers Gateway использует Codex {version}.",
  "startup.codexRuntime.clampHiddenWithEfforts": "Некоторые уровни рассуждений скрыты, потому что Univers Gateway использует Codex {version} (удалены: {efforts}).",
  "startup.codexRuntime.olderBinary": "Univers Gateway использует более старый бинарник Codex ({version}). Доступна более новая установка.",

  // dashboard
  "dash.subtitle": "Актуальное состояние локального прокси Univers Gateway, его провайдеров и моделей, маршрутизируемых в Codex.",
  ...
  "dash.codexAutoStart": "Запускать Univers Gateway вместе с Codex",
  ...
  "dash.shadowCallTooltip": "Codex App в фоновом режиме вызывает служебную модель для генерации заголовков тредов, сообщений коммитов и оркестрации навыков. Эта модель менялась между версиями клиента, поэтому Univers Gateway перехватывает весь набор: {models}. Включите функцию, чтобы перенаправлять такие вызовы на выбранную вами модель.",
  ...
  "dash.maintenanceHint": "Обновите каталог моделей Codex или установите более новую версию Univers Gateway.",
  ...
  "dash.projectConfigTitle": "Конфигурация Codex в проекте обходит Univers Gateway",
  "dash.projectConfigHint": "Эти локальные настройки репозитория переопределяют прокси Univers Gateway (например, направляют запросы напрямую в OpenCode Go). Удалите их, чтобы в этом проекте действовала маршрутизация из ~/.codex/config.toml.",
  "dash.checkUpdate": "Проверить обновления",
  "dash.updateTitle": "Обновление Univers Gateway",
  ...
  "dash.updateStatus.running": "Обновление Univers Gateway.",
  ...
  // providers
  "prov.subtitle": "Настройте вышестоящих провайдеров, которых Univers Gateway маршрутизирует в Codex. Войдите в аккаунт, добавьте провайдера или отредактируйте конфигурацию вручную.",
  ...
  "oauthTos.anthropicBody": "Прямое повторное использование OAuth-токенов подписки Claude через сторонний прокси, такой как Univers Gateway, не является поддерживаемой интеграцией Anthropic и может привести к ограничению доступа. Поддерживаемые интеграции Agent SDK, использующие подписки Claude, — это отдельный механизм.",
  "oauthTos.highBody": "Univers Gateway подключает {provider} через сторонний механизм OAuth. Неподдерживаемое использование может привести к ограничению или приостановке доступа.",
  "oauthTos.elevatedBody": "Univers Gateway подключает {provider} через неофициальный механизм OAuth. По возможности используйте официальный клиент; нетипичный или автоматизированный трафик может быть расценён как злоупотребление, и доступ может быть ограничен или приостановлен.",
  "oauthTos.saferPath": "Более безопасный вариант: вместо этого настройте API-ключ в Univers Gateway.",
  ...
  "models.subtitle": "Управляйте тем, какие модели видит Codex — нативные GPT (сквозной проброс) и модели маршрутизируемых провайдеров, сгруппированные по провайдеру (нажмите на заголовок, чтобы свернуть группу). Скрытые модели исчезают из каталога и селектора, но остаются вызываемыми по точному id. Изменения применяются на следующем ходе Codex — Univers Gateway сбрасывает 5-минутный кэш моделей Codex, поэтому перезапуск не требуется.",
  ...
  "logs.subtitle": "Недавние запросы через локальный прокси Univers Gateway, новые сверху.",
  ...
  "usage.refresh": "Обновить использование",
  "usage.refreshing": "Обновление использования…",
  "usage.range.all": "Все",
  ...
  "usage.range.1d": "День",
  "usage.range.30d": "Месяц",
  "usage.range.7d": "Неделя",
  "usage.window.older": "Предыдущий период",
  "usage.window.newer": "Следующий период",
  ...
  "usage.section.keys": "Ключи доступа",
  ...
  "usage.filter.key.label": "Ключ доступа",
  "usage.filter.key.all": "Все ключи",
  "usage.filter.key.clear": "Сбросить фильтр по ключу",
  "usage.key.none": "Без ключа доступа",
  "usage.keys.rangeLimited": "Разбивка по ключам доступна для дневного и недельного окон. Месяц и всё время агрегируются без измерения ключа.",
  "usage.col.requests": "Запросы",
  "usage.col.measured": "Измерено",
  "usage.col.reported": "Сообщено",
  "usage.col.tokens": "Токены",
  "usage.col.key": "Ключ",
  "usage.col.cost": "Оценочная стоимость",
  "usage.col.unpriced": "Без цены",
  ...
  "quota.weeklyRemaining": "Осталось {pct}% из недели",
  "quota.monthlyRemaining": "Осталось {pct}% из 30 дн.",
  ...
  "integrations.subtitle": "Подключайте клиенты к Univers Gateway, управляйте учётными данными и восстанавливайте конфигурацию клиентов.",
  ...
  "integrations.codex.body": "Подключением Codex управляет прокси-сервис. При запуске Univers Gateway оно применяется, а при остановке сервиса восстанавливается нативная маршрутизация.",
  ...
  "integrations.dialog.desktop.changes": "Если {path} содержит профиль шлюза, управляемый Univers Gateway, Desktop сначала выберет новый стандартный профиль без учётных данных, а затем удалит старый профиль и резервную копию.",
  "integrations.dialog.desktop.breakage": "Claude Desktop вернётся к обычному Claude вместо моделей, маршрутизируемых через Univers Gateway.",
  "integrations.dialog.desktop.undo": "При повторном включении профиль Univers Gateway будет создан заново из сохранённых назначений моделей.",
  ...
  "integrations.native.error.desktopCleanupIncomplete": "Claude Desktop указывает на стандартный режим, но старые файлы учётных данных Univers Gateway остались в: {paths}.",
  ...
  "integrations.detail.grokAbsent": "В конфигурации нет блока Univers Gateway",
  "integrations.dialog.grok.title": "Отключить интеграцию Grok Build?",
  "integrations.dialog.grok.changes": "Из {path} будет удалён только блок, отмеченный Univers Gateway. Содержимое, добавленное вручную вне блока, останется без изменений.",
  "integrations.dialog.grok.breakage": "После отключения псевдонимы моделей Univers Gateway исчезнут из Grok Build. Модели, использовавшиеся с учётной записью xAI, останутся доступны.",
  "integrations.dialog.grok.undo": "Если Univers Gateway запущен на loopback-адресе, при повторном включении будет записан новый блок из списка доступных на тот момент моделей.",
  "integrations.dialog.grok.confirm": "Отключить",
  "integrations.native.msg.nonLoopbackRemoved": "Grok Build можно регистрировать автоматически, только когда Univers Gateway запущен на loopback-адресе. Предыдущий блок, указывавший на loopback-адрес, удалён.",
  "integrations.native.msg.nonLoopbackRemovedNoop": "Grok Build можно регистрировать автоматически, только когда Univers Gateway запущен на loopback-адресе. Предыдущего блока для удаления не было.",
  "integrations.native.msg.nonLoopbackSuperseded": "Grok Build можно регистрировать автоматически, только когда Univers Gateway запущен на loopback-адресе. Тем временем другая программа записала в конфигурацию новый блок, поэтому текущий блок в файле создан не этим запросом.",
  "integrations.native.error.orphanedMarker": "В {path} есть начальная метка Univers Gateway, но нет конечной. Файл не изменён, потому что невозможно надёжно определить конец блока.",
  ...
  "integrations.onboarding": "При применении сначала сохраняется резервная копия, а затем записывается один блок провайдера Univers Gateway. При отключении удаляется только этот блок, а сохранённый снимок можно восстановить.",
  "integrations.empty.title": "Установленные клиенты не обнаружены",
  "integrations.empty.body": "Установите поддерживаемый клиент, затем вернитесь сюда и примените Univers Gateway.",
  ...
  "integrations.error.conflict": "После записи Univers Gateway конфигурация изменилась. Ничего не было удалено.",
  ...
  "integrations.bulk.body": "Будет удалён только блок, принадлежащий Univers Gateway. Для каждого клиента предварительно сохраняется снимок исходного состояния.",
  ...
  "codexAuth.autoSwitchQuotaDesc": "Квота всегда распределяет новые и непривязанные задачи по сроку сброса. При использовании {threshold}% или выше привязанная задача также может перейти при следующем запросе; Go/Free используют только 30 дней.",
  ...
  "accountPool.strategyHintQuota": "Квота распределяет каждую новую или непривязанную задачу по сроку сброса; порог использования управляет только переносом уже привязанной задачи.",
  ...
  "api.subtitle": "Сгенерированные API-ключи дают внешним приложениям доступ к прокси Univers Gateway. Аутентификация — через заголовок {authHeader}; какие заголовки принимает каждый эндпоинт, смотрите в таблице ниже.",
  ...
  "claude.smallFastModelNativeWarning": "Если модель не выбрана, Univers Gateway не задаёт переопределения вспомогательной модели. Claude Code может использовать нативную модель Sonnet, что может привести к расходам у вашего нативного провайдера.",
  ...
  "grok.subtitle": "Модели, зарегистрированные Univers Gateway в вашей конфигурации Grok.",
  ...
  "grok.notConfiguredHint": "Установите Grok и перезапустите прокси — Univers Gateway запишет управляемый блок в:",
  ...
  "cws.howBody": "Запросите у Codex публичное имя модели комбо. Если оно не задано, используется combo/<id>. Univers Gateway выбирает цель и переключается на следующую только при сбоях вышестоящего провайдера, допускающих повтор. Если доступных целей не осталось, запрос завершается ошибкой, а не переходит на глобальный провайдер по умолчанию.",
