...
export const ja: Record<TKey, string> = {
  ...
  "common.retry": "再試行",
  "auth.adminTokenTitle": "Univers Gateway 管理者トークン (OPENCODEX_ADMIN_AUTH_TOKEN)",
  ...
  "app.logoAria": "Univers Gateway ロゴ",
  ...
  // startup health
  "startup.title": "起動安全性",
  "startup.subtitle": "再起動後にローカルプロキシへの接続が再接続ループになる前に、Codex が Univers Gateway へ到達できるか確認します。",
  "startup.refresh": "更新",
  ...
  "startup.summary.protected": "再起動後も Univers Gateway を利用できます",
  ...
  "startup.riskDetailCustomLocal": "Codex はカスタムローカルゲートウェイを参照しています。Univers Gateway はその再起動ライフサイクルを管理・検証できません。",
  ...
  "startup.codexRuntime.clampHidden": "Univers Gateway が Codex {version} を使用したため、一部の reasoning effort オプションが非表示になりました。",
  "startup.codexRuntime.clampHiddenWithEfforts": "Univers Gateway が Codex {version} を使用したため、一部の reasoning effort オプションが非表示になりました（削除: {efforts}）。",
  "startup.codexRuntime.olderBinary": "Univers Gateway は古い Codex バイナリ（{version}）を使用しています。より新しいインストールが利用可能です。",

  // dashboard
  "dash.subtitle": "ローカル Univers Gateway プロキシ、そのプロバイダー、Codex にルーティングされるモデルのライブ状態です。",
  ...
  "dash.stopping": "停止中…",
  "dash.codexAutoStart": "Codex と一緒に Univers Gateway を起動",
  ...
  "dash.shadowCallTooltip": "Codex App はスレッドタイトル生成、コミットメッセージ生成、スキルオーケストレーションをバックグラウンドで呼び出します。使われるモデルはクライアントのバージョンによって変わるため、Univers Gateway は {models} をまとめて傍受します。これをオンにすると、それらの呼び出しを選択したモデルにリダイレクトします。",
  ...
  "dash.maintenanceHint": "Codex のモデルカタログを更新するか、より新しい Univers Gateway リリースをインストールします。",
  ...
  "dash.projectConfigTitle": "プロジェクトの Codex 設定が Univers Gateway をバイパスします",
  "dash.projectConfigHint": "これらのリポジローカル設定は Univers Gateway プロキシを上書きします(例: OpenCode Go に直接ルーティング)。~/.codex/config.toml のルーティングがそのプロジェクトで適用されるように削除してください。",
  "dash.checkUpdate": "更新を確認",
  "dash.updateTitle": "Univers Gateway を更新",
  ...
  "dash.updateStatus.running": "Univers Gateway を更新しています。",
  ...
  // providers
  "prov.subtitle": "Univers Gateway が Codex にルーティングする上流プロバイダーを設定します。アカウントでログインするか、プロバイダーを追加、または生の設定を編集します。",
  "prov.add": "プロバイダーを追加",
  ...
  "oauthTos.anthropicBody": "Univers Gateway のような第三者プロキシ経由で Claude サブスクリプションの OAuth トークンを直接再利用することは、Anthropic がサポートする統合ではなく、アクセス制限につながる可能性があります。Claude サブスクリプションを使用するサポートされた Agent SDK 統合は別物です。",
  "oauthTos.highBody": "Univers Gateway は第三者 OAuth パス経由で {provider} に接続します。サポート外の利用はアクセス制限や停止につながる可能性があります。",
  "oauthTos.elevatedBody": "Univers Gateway は非公式 OAuth パス経由で {provider} に接続します。可能な場合は公式クライアントを使用してください。異常または自動化されたトラフィックは悪用とみなされ、アクセスが制限または停止される可能性があります。",
  "oauthTos.saferPath": "より安全な選択肢: 代わりに Univers Gateway で API キーを設定してください。",
  ...
  "models.subtitle": "Codex に表示するモデルを切り替えます — ネイティブ GPT パススルーとルーティングプロバイダー、プロバイダー別(ヘッダーをクリックで折りたたみ)。非表示モデルはカタログとピッカーから外れますが、正確な id での直接呼び出しは可能です。変更は次回の Codex ターンで適用 — Univers Gateway は Codex の 5 分間モデルキャッシュを無効化するので再起動は不要です。",
  ...
  // subagents
  "sub.subtitle": "Codex の {cmd} は最初の 5 モデル(優先度順)のみをオーバーライドとして通知します。ここで最大 5 つを選んでください — ネイティブ gpt またはルーティング — Univers Gateway がカタログ優先度を設定し、これらが先頭に来るようにします。他のモデルも正確な名前で呼び出し可能です; これは表示のみを制御します。",
  "sub.featured": "おすすめ",
  ...
  "logs.tabLogs": "ログ",
  "logs.tabDebug": "デバッグ",
  "logs.subtitle": "ローカル Univers Gateway プロキシを経由した最近のリクエスト(新しい順)。",
  ...
  "usage.range.30d": "月",
  "usage.range.7d": "週",
  "usage.window.older": "前の期間",
  "usage.window.newer": "次の期間",
  ...
  "usage.filter.key.label": "アクセスキー",
  "usage.filter.key.all": "すべてのキー",
  "usage.filter.key.clear": "キーフィルターを解除",
  "usage.key.none": "キーなし",
  "usage.keys.rangeLimited": "キー別の内訳は「日」「週」の期間のみ対応しています。「月」「全期間」はキー次元なしで集計されます。",
  "usage.col.requests": "リクエスト",
  "usage.col.measured": "計測",
  "usage.col.reported": "報告",
  "usage.col.tokens": "トークン",
  "usage.col.key": "キー",
  "usage.col.cost": "推定コスト",
  "usage.col.unpriced": "価格未設定",
  "usage.col.share": "割合",
  ...
  "quota.weeklyRemaining": "週間の残り {pct}%",
  "quota.monthlyRemaining": "30日の残り {pct}%",
  ...
  "integrations.subtitle": "クライアントを Univers Gateway に接続し、認証情報の管理とクライアント設定の復元を行います。",
  ...
  "integrations.codex.body": "Codex の接続はプロキシサービスが管理します。Univers Gateway を起動すると適用され、サービスを停止するとネイティブのルーティングに戻ります。",
  ...
  "integrations.dialog.desktop.changes": "{path} に Univers Gateway 管理のゲートウェイプロファイルがある場合、Desktop は先に認証情報のない標準プロファイルを選択し、その後で古いプロファイルとバックアップを削除します。",
  "integrations.dialog.desktop.breakage": "Claude Desktop は Univers Gateway 経由のモデルではなく、標準の Claude に戻ります。",
  "integrations.dialog.desktop.undo": "再度有効にすると、保存済みのモデル割り当てから Univers Gateway プロファイルを再生成します。",
  ...
  "integrations.native.error.desktopCleanupIncomplete": "Claude Desktop は標準モードを指していますが、古い Univers Gateway 認証情報ファイルが残っています: {paths}。",
  ...
  "integrations.detail.grokAbsent": "設定に Univers Gateway ブロックがありません",
  "integrations.dialog.grok.title": "Grok Build 連携を解除しますか？",
  "integrations.dialog.grok.changes": "{path} から、Univers Gateway が印を付けたブロックだけを削除します。ブロック外に直接書いた内容はそのまま残します。",
  "integrations.dialog.grok.breakage": "解除すると、Grok Build から Univers Gateway のモデルエイリアスが消えます。xAI アカウントで使用していたモデルはそのままです。",
  "integrations.dialog.grok.undo": "Univers Gateway が loopback アドレスで実行中なら、再び有効にしたとき、現在利用できるモデル一覧からブロックを新しく書き込みます。",
  "integrations.dialog.grok.confirm": "解除",
  "integrations.native.msg.nonLoopbackRemoved": "Grok Build を自動登録できるのは、Univers Gateway が loopback アドレスで実行中の場合だけです。loopback アドレスを指していた以前のブロックを削除しました。",
  "integrations.native.msg.nonLoopbackRemovedNoop": "Grok Build を自動登録できるのは、Univers Gateway が loopback アドレスで実行中の場合だけです。削除する以前のブロックはありませんでした。",
  "integrations.native.msg.nonLoopbackSuperseded": "Grok Build を自動登録できるのは、Univers Gateway が loopback アドレスで実行中の場合だけです。その間に別の場所から設定へ新しいブロックが書かれたため、現在ファイルにあるブロックはこのリクエストが作成したものではありません。",
  "integrations.native.error.orphanedMarker": "{path} に Univers Gateway の開始マーカーはありますが、終了マーカーがありません。ブロックの終端を特定できないため、ファイルは変更していません。",
  ...
  "integrations.onboarding": "適用時は、先にバックアップを保存してから Univers Gateway プロバイダーブロックを 1 つだけ書き込みます。無効化時はそのブロックだけを削除し、保存されたスナップショットから復元できます。",
  "integrations.empty.title": "インストール済みのクライアントが検出されませんでした",
  "integrations.empty.body": "対応クライアントをインストールしてから、ここに戻って Univers Gateway を適用してください。",
  ...
  "integrations.error.conflict": "Univers Gateway の書き込み後に設定が変更されました。何も削除していません。",
  ...
  "integrations.bulk.body": "Univers Gateway が所有するブロックだけを削除します。各クライアントについて、変更前のスナップショットを保存します。",
  ...
  "codexAuth.autoSwitchQuotaDesc": "クォータは新規/未紐付けタスクを常に期限ベースで割り当てます。使用率が {threshold}% 以上になると、紐付け済みタスクも次のリクエストで移動する場合があります。Go/Free は 30 日枠のみを使用します。",
  ...
  "accountPool.strategyHintQuota": "クォータ戦略は新規/未紐付けタスクを期限ベースで割り当て、使用量しきい値は紐付け済みタスクの移行だけを制御します。",
  ...
  // api access page
  "api.title": "API アクセス",
  "api.subtitle": "生成した API キーで外部アプリから Univers Gateway プロキシに接続します。認証は {authHeader} ヘッダーで行い、エンドポイントごとに受け付けるヘッダーは下の表のとおりです。",
  ...
  "claude.smallFastModelNativeWarning": "未設定の場合、Univers Gateway はヘルパーモデルの上書きを設定しません。Claude Code がネイティブの Sonnet モデルを使用し、ネイティブプロバイダーで料金が発生する可能性があります。",
  ...
  "grok.subtitle": "Univers Gateway が Grok 設定に登録したモデルです。",
  "grok.loading": "Grok の状態を読み込み中…",
  "grok.loadFail": "Grok 設定を読み取れませんでした。",
  "grok.notConfiguredTitle": "Grok Build が未設定です",
  "grok.notConfiguredHint": "Grok をインストールしてプロキシを再起動すると、Univers Gateway が管理ブロックを次の場所に書き込みます:",
  ...
  "cws.howTitle": "仕組み",
  "cws.howBody": "Codex に combo/<id> を要求します。Univers Gateway はターゲットを選び、再試行可能な上流の失敗時のみホップします。利用可能なターゲットが残っていない場合、グローバルなデフォルトプロバイダーを使わずにフェイルクローズします。",
