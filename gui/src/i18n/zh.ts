...
export const zh: Record<TKey, string> = {
  ...
  "common.loading": "加载中…",
  "common.retry": "重试",
  "auth.adminTokenTitle": "Univers Gateway 管理员令牌 (OPENCODEX_ADMIN_AUTH_TOKEN)",
  ...
  // startup health
  "startup.title": "启动安全",
  "startup.subtitle": "检查重启后 Codex 是否仍能连接 Univers Gateway，避免本地代理路由陷入重复重连。",
  "startup.refresh": "刷新",
  ...
  "startup.summary.protected": "重启后 Univers Gateway 会自动可用",
  "startup.summary.atRisk": "重启后 Codex 可能无法访问模型",
  "startup.riskDetail": "Codex 已指向本地代理，但没有持久服务或正常的 launcher shim 将其重新启动。",
  "startup.riskDetailCustomLocal": "Codex 指向自定义本地网关。Univers Gateway 无法管理或验证该网关的重启生命周期。",
  ...
  "startup.codexRuntime.clampHidden": "部分推理强度选项已隐藏，因为 Univers Gateway 正在使用 Codex {version}。",
  "startup.codexRuntime.clampHiddenWithEfforts": "部分推理强度选项已隐藏，因为 Univers Gateway 正在使用 Codex {version}（已移除：{efforts}）。",
  "startup.codexRuntime.olderBinary": "Univers Gateway 正在使用较旧的 Codex 二进制文件（{version}）。检测到可用的较新安装。",

  // dashboard
  "dash.subtitle": "本地 Univers Gateway 代理、其提供方以及路由到 Codex 的模型的实时状态。",
  ...
  "dash.stopping": "正在停止…",
  "dash.codexAutoStart": "随 Codex 启动 Univers Gateway",
  ...
  "dash.shadowCallTooltip": "Codex 应用会在后台调用辅助模型来生成线程标题、提交消息以及进行技能编排。该模型随客户端版本变化，因此 Univers Gateway 会同时拦截这些模型：{models}。启用此选项可将这些调用重定向到您选择的模型。",
  ...
  "dash.effortCapNone": "无上限",
    "dash.maintenance": "维护",
  "dash.maintenanceHint": "刷新 Codex 模型目录，或安装新的 Univers Gateway 版本。",
  ...
  "dash.projectConfigTitle": "项目 Codex 配置绕过了 Univers Gateway",
  "dash.projectConfigHint": "这些仓库级设置会覆盖 Univers Gateway 代理（例如直接走 OpenCode Go）。请移除它们，以便该项目使用 ~/.codex/config.toml 的代理路由。",
  "dash.checkUpdate": "检查更新",
  "dash.updateTitle": "更新 Univers Gateway",
  ...
  "dash.updateStatus.running": "正在更新 Univers Gateway。",
  ...
  // providers
  "prov.subtitle": "配置 Univers Gateway 路由到 Codex 的上游提供方。使用账户登录、添加提供方，或编辑原始配置。",
  "prov.add": "添加提供方",
  ...
  "oauthTos.anthropicBody": "通过 Univers Gateway 等第三方代理直接复用 Claude 订阅 OAuth 令牌，并非 Anthropic 支持的集成方式，可能导致访问受限。可使用 Claude 订阅的受支持 Agent SDK 集成属于另一种方式。",
  "oauthTos.highBody": "Univers Gateway 通过第三方 OAuth 路径连接 {provider}。如果该用法不受支持，访问可能会被限制或暂停。",
  "oauthTos.elevatedBody": "Univers Gateway 通过非官方 OAuth 路径连接 {provider}。请尽量使用官方客户端；异常或自动化流量可能被视为滥用，访问可能会被限制或暂停。",
  "oauthTos.saferPath": "更安全的做法：改为在 Univers Gateway 中配置 API 密钥。",
  ...
  "models.subtitle": "开关 Codex 可见的模型 — 原生 GPT passthrough 与已路由模型按提供方分组（点击标题可折叠）。隐藏的模型不会出现在目录和模型选择器中，但仍可按精确 id 直接调用。更改在下一个 Codex 回合生效 — Univers Gateway 会使 Codex 的 5 分钟模型缓存失效，因此无需重启。",
  ...
  // subagents
  "sub.subtitle": "Codex 的 {cmd} 仅将优先级最高的前 5 个模型作为覆盖项公开。在此最多选择 5 个 — 原生 gpt 或已路由模型 — Univers Gateway 会设置它们的目录优先级，使其正好排在前面。其他模型仍可按确切名称调用；此设置仅控制显示项。",
  "sub.featured": "精选",
  ...
  "logs.tabLogs": "日志",
  "logs.tabDebug": "调试",
  "logs.subtitle": "经过本地 Univers Gateway 代理的最近请求，最新在前。",
  ...
  "usage.range.30d": "月",
  "usage.range.7d": "周",
  "usage.window.older": "上一周期",
  "usage.window.newer": "下一周期",
  ...
  "usage.filter.key.label": "准入密钥",
  "usage.filter.key.all": "全部密钥",
  "usage.filter.key.clear": "清除密钥筛选",
  "usage.key.none": "未使用准入密钥",
  "usage.keys.rangeLimited": "按密钥的明细仅覆盖「日」和「周」窗口；「月」和「全部」是聚合口径，不含密钥维度。",
  ...
  "nav.openMenu": "打开菜单",
  "nav.closeMenu": "关闭菜单",
  "integrations.subtitle": "将客户端连接到 Univers Gateway，管理凭据并恢复客户端配置。",
  ...
  "integrations.codex.body": "Codex 连接由代理服务管理。启动 Univers Gateway 时应用该连接；停止服务时恢复原生路由。",
  ...
  "integrations.dialog.desktop.changes": "如果 {path} 包含由 Univers Gateway 管理的网关配置，Desktop 会先选择新的无凭据标准配置，再移除旧配置及其备份。",
  "integrations.dialog.desktop.breakage": "Claude Desktop 将不再使用经由 Univers Gateway 路由的模型，而会恢复为标准 Claude。",
  "integrations.dialog.desktop.undo": "重新开启后，会根据已保存的模型分配重新生成 Univers Gateway 配置。",
  ...
  "integrations.native.error.desktopCleanupIncomplete": "Claude Desktop 已指向标准模式，但仍残留旧的 Univers Gateway 凭据文件：{paths}。",
  ...
  "integrations.detail.grokAbsent": "配置中没有 Univers Gateway 区块",
  "integrations.dialog.grok.title": "要停用 Grok Build 集成吗？",
  "integrations.dialog.grok.changes": "只会从 {path} 中删除由 Univers Gateway 标记的区块。区块之外手动写入的内容将保持不变。",
  "integrations.dialog.grok.breakage": "停用后，Grok Build 中的 Univers Gateway 模型别名将消失。通过 xAI 账号使用的模型不受影响。",
  "integrations.dialog.grok.undo": "如果 Univers Gateway 正在 loopback 地址上运行，再次启用时会根据当前可用的模型列表重新写入区块。",
  "integrations.dialog.grok.confirm": "停用",
  "integrations.native.msg.nonLoopbackRemoved": "只有当 Univers Gateway 在 loopback 地址上运行时，才能自动注册 Grok Build。已删除之前指向 loopback 地址的区块。",
  "integrations.native.msg.nonLoopbackRemovedNoop": "只有当 Univers Gateway 在 loopback 地址上运行时，才能自动注册 Grok Build。没有需要删除的旧区块。",
  "integrations.native.msg.nonLoopbackSuperseded": "只有当 Univers Gateway 在 loopback 地址上运行时，才能自动注册 Grok Build。在此期间，其他进程向配置写入了新区块，因此文件中的当前区块并非由本次请求创建。",
  "integrations.native.error.orphanedMarker": "{path} 中有 Univers Gateway 开始标记，但没有结束标记。由于无法确定该区块的结束位置，因此未修改文件。",
  ...
  "integrations.onboarding": "应用时会先保存备份，再写入一个 Univers Gateway 提供方配置块。禁用时只移除该配置块，并可从保留的快照恢复。",
  "integrations.empty.title": "未检测到已安装的客户端",
  "integrations.empty.body": "安装受支持的客户端，然后返回此处应用 Univers Gateway。",
  ...
  "integrations.error.conflict": "Univers Gateway 写入后配置又发生了更改。未移除任何内容。",
  ...
  "integrations.bulk.body": "只会移除归 Univers Gateway 所有的配置块。每个客户端都会先保留一份写入前快照。",
  ...
  "codexAuth.autoSwitchQuotaDesc": "配额会始终按期限调度新建/未绑定任务；达到或超过 {threshold}% 后，已绑定任务也可能在下一次请求时迁移。Go/Free 仅使用 30 天窗口。",
  ...
  "accountPool.strategyHintQuota": "配额策略会按期限调度每个新建/未绑定任务；用量阈值只控制已有绑定任务是否迁移。",
  ...
  // api access page
  "api.title": "API 访问",
  "api.subtitle": "用生成的 API 密钥从外部应用访问 Univers Gateway 代理。认证使用 {authHeader} 请求头；各端点接受哪些请求头见下表。",
  ...
  "claude.smallFastModelNativeWarning": "留空时，Univers Gateway 不会设置辅助模型覆盖项。Claude Code 可能使用其原生 Sonnet 模型，并可能产生原生提供方费用。",
  ...
  "common.close": "关闭",
  "common.ok": "确定",
  "app.logoAria": "Univers Gateway 徽标",
 ...
 "modal.baseUrlPlaceholder": "https://...",
  ...
  "cws.howTitle": "工作原理",
  "cws.howBody": "在 Codex 中请求组合的公开模型名称；未设置时默认使用 combo/<id>。Univers Gateway 仅在可重试的上游错误时切换目标。若没有可用目标，请求会直接失败，不会回退到全局默认提供方。",
  ...
  "grok.subtitle": "Univers Gateway 已注册到你的 Grok 配置中的模型。",
  "grok.loading": "正在加载 Grok 状态…",
  "grok.loadFail": "无法读取 Grok 配置。",
  "grok.notConfiguredTitle": "Grok Build 尚未接入",
  "grok.notConfiguredHint": "安装 Grok 后重启代理，Univers Gateway 会把托管块写入：",
  "grok.endpoint": "端点",
  "grok.colModel": "模型",
