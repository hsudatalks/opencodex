...
const STATUS_CODES: Record<number, LocalizedInfo> = {
  ...
  401: {
    en: { label: "Unauthorized", description: "Credentials are missing, expired, or invalid. Re-login or refresh the account/provider credentials used by Univers Gateway." },
    ko: { label: "인증 필요", description: "자격 증명이 없거나 만료되었거나 유효하지 않습니다. Univers Gateway에서 사용하는 계정 또는 제공자 자격 증명을 다시 로그인하거나 갱신해야 합니다." },
    zh: { label: "未授权", description: "凭据缺失、已过期或无效。请重新登录，或刷新 Univers Gateway 使用的账号/提供商凭据。" },
    de: { label: "Nicht autorisiert", description: "Anmeldedaten fehlen, sind abgelaufen oder ungültig. Melde dich erneut an oder aktualisiere die von Univers Gateway genutzten Konto-/Anbieter-Zugangsdaten." },
    ru: { label: "Не авторизован", description: "Учётные данные отсутствуют, истекли или недействительны. Войдите заново или обновите учётные данные аккаунта или провайдера, которые использует Univers Gateway." },

    ja: { label: "認証が必要", description: "認証情報が不在・期限切れ・無効です。Univers Gateway が使用するアカウントまたはプロバイダー認証情報を再ログインまたは更新してください。" },
  },
  402: {
  ...
  404: {
    en: { label: "Not found", description: "The requested route, model, account, or upstream resource was not found. Verify the model name and Univers Gateway provider configuration." },
    ko: { label: "찾을 수 없음", description: "요청한 경로, 모델, 계정 또는 업스트림 리소스를 찾을 수 없습니다. 모델 이름과 Univers Gateway 제공자 설정을 확인해야 합니다." },
    zh: { label: "未找到", description: "找不到请求的路由、模型、账号或上游资源。请确认模型名称和 Univers Gateway 提供商配置。" },
    de: { label: "Nicht gefunden", description: "Die angeforderte Route, das Modell, das Konto oder die Upstream-Ressource wurde nicht gefunden. Prüfe Modellname und Univers Gateway-Anbieterkonfiguration." },
    ru: { label: "Не найдено", description: "Запрошенный маршрут, модель, аккаунт или вышестоящий ресурс не найден. Проверьте имя модели и конфигурацию провайдера в Univers Gateway." },

    ja: { label: "見つかりません", description: "要求されたルート、モデル、アカウント、上流リソースが見つかりませんでした。モデル名と Univers Gateway プロバイダー設定を確認してください。" },
  },
  408: {
  ...
  424: {
    en: { label: "Provider dependency failed", description: "A required upstream dependency failed while Univers Gateway was routing the request. Retry later or switch to another configured provider." },
    ko: { label: "제공자 의존성 실패", description: "Univers Gateway가 요청을 라우팅하는 동안 필요한 업스트림 의존성이 실패했습니다. 나중에 재시도하거나 다른 설정된 제공자로 전환해야 합니다." },
    zh: { label: "提供商依赖失败", description: "Univers Gateway 路由请求时，必需的上游依赖失败。请稍后重试，或切换到另一个已配置的提供商。" },
    de: { label: "Anbieter-Abhängigkeit fehlgeschlagen", description: "Eine erforderliche Upstream-Abhängigkeit ist fehlgeschlagen, während Univers Gateway die Anfrage geroutet hat. Später erneut versuchen oder zu einem anderen Anbieter wechseln." },
    ru: { label: "Сбой зависимости провайдера", description: "Необходимая вышестоящая зависимость дала сбой, пока Univers Gateway маршрутизировал запрос. Повторите попытку позже или переключитесь на другого настроенного провайдера." },

    ja: { label: "プロバイダー依存の失敗", description: "Univers Gateway がリクエストをルーティング中に必要な上流依存が失敗しました。後で再試行するか、別の設定済みプロバイダーに切り替えてください。" },
  },
  429: {
  ...
  499: {
    en: { label: "Client closed request", description: "The client disconnected or canceled the request before Univers Gateway finished routing it. Retry if the cancellation was accidental." },
    ko: { label: "클라이언트 취소", description: "Univers Gateway가 라우팅을 끝내기 전에 클라이언트 연결이 끊기거나 요청이 취소되었습니다. 의도한 취소가 아니면 다시 시도해야 합니다." },
    zh: { label: "客户端已取消", description: "Univers Gateway 完成路由前，客户端已断开连接或取消请求。如果不是有意取消，请重试。" },
    de: { label: "Client hat Anfrage geschlossen", description: "Der Client hat die Verbindung getrennt oder die Anfrage abgebrochen, bevor Univers Gateway das Routing abgeschlossen hat. Bei versehentlichem Abbruch erneut versuchen." },
    ru: { label: "Запрос закрыт клиентом", description: "Клиент отключился или отменил запрос до того, как Univers Gateway завершил его маршрутизацию. Если отмена была случайной, повторите попытку." },

    ja: { label: "クライアントがリクエストをクローズ", description: "Univers Gateway がルーティングを終える前にクライアントが切断またはキャンセルしました。意図しないキャンセルなら再試行してください。" },
  },
  500: {
    en: { label: "Proxy error", description: "Univers Gateway hit an internal error while handling the request. Retry once, then check proxy logs if it repeats." },
    ko: { label: "프록시 오류", description: "Univers Gateway가 요청을 처리하는 동안 내부 오류가 발생했습니다. 한 번 재시도하고 반복되면 프록시 로그를 확인해야 합니다." },
    zh: { label: "代理错误", description: "Univers Gateway 处理请求时发生内部错误。请先重试一次；如果重复出现，请检查代理日志。" },
    de: { label: "Proxy-Fehler", description: "Univers Gateway ist bei der Anfragebearbeitung auf einen internen Fehler gestoßen. Einmal erneut versuchen, bei Wiederholung Proxy-Logs prüfen." },
    ru: { label: "Ошибка прокси", description: "В Univers Gateway произошла внутренняя ошибка при обработке запроса. Повторите попытку один раз; если ошибка повторяется, проверьте логи прокси." },

    ja: { label: "プロキシエラー", description: "Univers Gateway がリクエスト処理中に内部エラーに遭遇しました。1 回再試行し、繰り返す場合はプロキシログを確認してください。" },
  },
  502: {
...
const GENERIC_STATUS: { client: LocalizedInfo; server: LocalizedInfo } = {
  ...
  server: {
    en: { label: "Server or upstream error", description: "Univers Gateway or an upstream provider failed while processing the request. Retry later or route the request to another provider." },
    ko: { label: "서버 또는 업스트림 오류", description: "Univers Gateway 또는 업스트림 제공자가 요청 처리 중 실패했습니다. 나중에 재시도하거나 다른 제공자로 라우팅해야 합니다." },
    zh: { label: "服务器或上游错误", description: "Univers Gateway 或上游提供商处理请求时失败。请稍后重试，或将请求路由到其他提供商。" },
    de: { label: "Server- oder Upstream-Fehler", description: "Univers Gateway oder ein Upstream-Anbieter ist bei der Anfragebearbeitung fehlgeschlagen. Später erneut versuchen oder zu einem anderen Anbieter routen." },
    ru: { label: "Ошибка сервера или провайдера", description: "Univers Gateway или вышестоящий провайдер завершил обработку запроса с ошибкой. Повторите попытку позже или направьте запрос другому провайдеру." },

    ja: { label: "サーバーまたは上流エラー", description: "Univers Gateway または上流プロバイダーがリクエスト処理中に失敗しました。後で再試行するか、リクエストを別のプロバイダーにルーティングしてください。" },
  },
};
