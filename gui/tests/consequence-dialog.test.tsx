...
test("renders the four Korean slots in order with the live path and no side effect", async () => {
  ...
  expect([title.textContent, ...paragraphs.map(paragraph => paragraph.textContent)]).toEqual([
    ...
    `${livePath}에서 Univers Gateway가 표시해 둔 블록만 제거합니다. 블록 바깥에 직접 쓴 내용은 그대로 둡니다.`,
    "해제하면 Grok Build에서 Univers Gateway 모델 별칭이 사라집니다. xAI 계정으로 쓰던 모델은 그대로입니다.",
    "Univers Gateway가 loopback 주소로 실행 중이면, 다시 켤 때 지금 쓸 수 있는 모델 목록으로 블록을 새로 씁니다.",
  ]);
