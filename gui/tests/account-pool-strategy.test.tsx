...
describe("AccountPoolStrategyControls", () => {
  test("renders strategy options and hides sticky unless round-robin", () => {
    ...
    expect(quota).toContain("Quota deadline-schedules every new/unbound task");
    expect(quota).toContain("usage threshold only controls migration of an existing bound task");
  ...
  test("keeps both the strategy description and the session-affinity notice", () => {
    ...
    expect(markup).toContain("How Univers Gateway assigns an account to a new/unbound task.");
