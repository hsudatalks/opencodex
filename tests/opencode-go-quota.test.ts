...
describe("OpenCode Go provider quota", () => {
  ...
  test("does not probe quota for a noncanonical OpenCode Go destination", async () => {
    ...
    globalThis.fetch = (async () => {
      fetchCalls += 1;
      return new Response(JSON.stringify({ usage: { rolling: { percent: 1 } } }), { status: 200 });
    }) as typeof fetch;
    const result = await fetchProviderQuotaReports(openCodeGoConfig("https://example.invalid/zen/go/v1"), true);
