...
| `src/server/evaluations.ts`, `src/evaluation/` | Opt-in TypeSafe typed evaluation: administrator-owned routes and credentials, bounded request/response validation, cancellation, single-attempt forwarding and usage attribution. `/v1/models?capability=evaluate` keeps non-chat models out of chat catalogs. |
