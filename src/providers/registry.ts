 ...
 * family above — the family is NOT a proxy for the capability. Shape discrimination (circle /
 * square / triangle on a 256x256 canvas, where a blind model scores about one in three by luck)
 * was run on 2026-09-22 against both transports this repo routes:
 *
 *   opencode-go/mimo-v2.6-flash                    6/6   declared here
 *   opencode-go/mimo-v2.6-pro                      6/6   declared here
 *   command-code/xiaomi/mimo-v2.6-flash            6/6   declared here
 *   command-code/xiaomi/mimo-v2.6-pro              6/6   declared here
 *   command-code/xiaomi/mimo-v2.6-pro-ultraspeed   6/6   declared here
 *
 * The two halves of the 2.x family land on opposite sides of the line and are declared per id:
 * `mimo-v2-pro` and `mimo-v2.5-pro` stay text-only (they are in `noVisionModels` below and route
 * through the vision sidecar), while every 2.6 id accepts images natively. A family-level table
 * would either hand the 2.6 capability to a model that cannot deliver it or strip images from one
 * that can.
 *
 * The two providers spell the same upstream ids differently — `opencode-go` serves them bare,
 * `command-code` vendor-namespaced on the `/alpha/generate` wire — so each list is spelled out
 * rather than derived from the other.
 *
 * Budget note, because it looks like blindness: at a small `max_tokens` these models spend the
...
export const PROVIDER_REGISTRY: readonly ProviderRegistryEntry[] = [
  ...
  },
  {
     ...
     *   xiaomi/mimo-v2.6-flash            6/6   declared
     *   xiaomi/mimo-v2.6-pro              6/6   declared
     *   xiaomi/mimo-v2.6-pro-ultraspeed   6/6   declared
     *
