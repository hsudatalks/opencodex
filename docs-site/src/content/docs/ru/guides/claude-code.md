...
используйте `/model <id>` внутри этого диалога. Univers Gateway не видит состояние селектора и
...
Если отсутствуют и `tierModels.haiku`, и `smallFastModel`, Univers Gateway оставляет обе переменные вспомогательной модели незаданными. Затем Claude Code выбирает нативную вспомогательную модель (сейчас Sonnet), что может привести к расходам у нативного провайдера.
