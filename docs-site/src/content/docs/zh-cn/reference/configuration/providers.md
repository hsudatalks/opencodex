...
| `autoSwitchThreshold?` | `number` | `80` | `quota` 新会话的额度余量过滤阈值，也是 `fill-first` 未绑定分配的耗尽点。它不会迁移正在执行的 turn；正常 `round-robin` 不使用它。分数取已知 5 小时、周或 30 天 quota window 的最高值。`0` 只关闭余量过滤，不关闭分配或故障恢复。 |
| `accountPoolStrategy?` | `"quota" \| "round-robin" \| "fill-first"` | `"quota"` | 新建/未绑定 Codex 请求的分配策略。没有 live `(parent thread id, quota scope)` affinity 的请求属于未绑定；代理重启或 affinity 重置后，已有可见任务也可能未绑定。`quota` 计算 `剩余额度百分比 × 144 / 距离周/30 天重置或最近手动重置到期的小时数`，按每 10 点分档，并在最高档的全部账号中按 affinity 数量负载均衡新会话；没有固定候选账号数量上限。缺少期限数据时回退到旧的 usage 排序。`round-robin` 均匀分配未绑定请求；`fill-first` 在 cooldown、不可用或耗尽阈值前持续分配给活跃账号。 |
...
| `accountMaxConcurrentTurns?` | `number` | `10` | 单个账号允许同时执行的 Codex turn 上限。这是最终准入保护：达到上限时返回短暂重试响应，不改变账号选择或 thread affinity。范围 1–100。 |
...
分为新建/未绑定任务分配、turn 结束后的 affinity 维护和故障恢复。已绑定任务在整个 turn 中保持
账号不变；turn 结束后，如果所在的 10 点紧迫度档比最高档落后超过 50 点，`quota` 可以释放绑定，
让下一次 turn 走正常的新会话分配。暂停、cooldown、重新认证和故障处理也能独立清除或改变 routing。
未绑定请求没有 live 账号绑定，也可能是代理重启或 affinity 重置后的已有任务。输出前的
...
**分配与主动切换策略：** `quota`（默认）使用周/30 天重置和最近手动重置到期时间中较早者，计算 `剩余额度百分比 × 144 / 距离重置小时数`，每 10 点分档，并在最高档全部账号中按已绑定会话数负载均衡新会话，不限制候选账号数量。缺少可比期限数据时回退到旧的 usage 排序。已有 affinity 在 turn 内保持稳定；turn 结束后，落后最高档超过 50 点的绑定会被释放，让下一 turn 重新分配。`autoSwitchThreshold` 只过滤新分配的额度余量。`round-robin` 均匀分配未绑定请求，用量
