...
| `autoSwitchThreshold?` | `number` | `80` | Usage headroom threshold for new-session eligibility under `quota` and the drain point for unbound `fill-first` assignment. It never moves an in-flight turn. Normal `round-robin` selection does not use it. The score uses the hottest known 5h, weekly, or 30d quota window. `0` disables this headroom filter, not assignment or failure recovery. |
| `accountPoolStrategy?` | `"quota" \| "round-robin" \| "fill-first"` | `"quota"` | Assignment strategy for new/unbound Codex requests. A request is unbound when it has no live (parent thread id, quota scope) affinity; a visible existing task can become unbound after proxy restart or affinity reset. `quota` computes `remaining percent × 144 / hours to the governing weekly/30-day reset or nearest manual-reset expiry`, groups accounts into 10-point urgency buckets, and balances new sessions by affinity count across every account in the highest bucket. If deadline data is unavailable it falls back to the legacy usage ranking. `round-robin` distributes unbound requests evenly; `fill-first` keeps assigning unbound requests to the active account until cooldown, unavailability, or the configured drain threshold. |
...
| `accountMaxConcurrentTurns?` | `number` | `10` | Maximum in-flight Codex turns per account. This is a final admission guard: saturation returns a short retry response without changing account choice or thread affinity. Range 1–100. |
...
separates new/unbound assignment, post-turn affinity maintenance, and failure recovery. A bound task
keeps its account throughout a turn. After settlement, `quota` may release the binding when its
10-point urgency bucket trails the highest bucket by more than 50 points; the next turn then performs
normal new-session assignment. Pause, cooldown, reauthentication, and failure handling can clear or
move routing independently. An unbound request has no live account binding; this can include an existing visible
...
| Strategy | Behaviour |
| --- | --- |
| `quota` (default) | Compute `remaining percent × 144 / hours to reset`, using the earlier of the governing weekly/30-day reset and nearest expiring manual reset credit. Group scores into 10-point buckets and balance new sessions by affinity count across all accounts in the highest bucket, with no fixed candidate-count cap. If comparable deadline data is unavailable, fall back to the legacy usage ranking. Existing affinity remains stable during a turn; after settlement, a binding more than 50 bucket points behind the leader is released so its next turn can be reassigned. `autoSwitchThreshold` filters headroom for new assignments only. |
