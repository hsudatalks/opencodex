...
| `accountPoolStrategy?` | `"quota" \| "round-robin" \| "fill-first"` | `"quota"` | Assignment strategy for new/unbound Codex requests. A request is unbound when it has no live (parent thread id, quota scope) affinity; a visible existing task can become unbound after proxy restart or affinity reset. `quota` ranks eligible accounts by remaining governing capacity per hour until the weekly/30-day reset or nearest manual-reset expiry, falling back to lowest usage when deadline data is unavailable. It keeps an eligible active account below `autoSwitchThreshold`, and after the threshold only moves to an account with lower usage. `round-robin` distributes unbound requests evenly; `fill-first` keeps assigning unbound requests to the active account until cooldown, unavailability, or the configured drain threshold. |
...
| `accountMaxConcurrentTurns?` | `number` | `10` | Maximum in-flight Codex turns per account. Capacity only affects new/unbound assignment and active upstream retries; it does not shorten the 24-hour idle thread affinity or move an existing thread to another account. Range 1–100. |
...
| Strategy | Behaviour |
| --- | --- |
| `quota` (default) | If no active account exists, rank eligible accounts by remaining governing capacity per hour until its weekly/30-day reset or nearest expiring manual reset credit. This spends capacity with the most urgent deadline first; if comparable deadline data is unavailable, it falls back to the lowest usage across known windows. Otherwise retain an eligible active account below `autoSwitchThreshold`; after it crosses the threshold, an unbound request or a bound task's next request can move only to a lower-usage eligible account. Existing thread affinity is not moved merely because another deadline is nearer. `0` disables this usage-driven re-evaluation, not failure recovery. |
